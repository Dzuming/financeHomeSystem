...
describe('ProductService', () => {
  ...
  it('should create product.service', inject([ProductService], (service: ProductService) => {
    expect(service instanceof ProductService).toBeTruthy();
  }));
  it('Should instantiate service with "new"', inject([Http], (http: Http) => {
    expect(http).not.toBeNull('http should be provided');
    let service = new ProductService(http);
    expect(service instanceof ProductService).toBe(true, 'new service should be ok');
  }));
  it('Should provide the mockBackend as XHRBackend',
    inject([XHRBackend], (backend: MockBackend) => {
      expect(backend).not.toBeNull('backend should be provided');
    }));
  it('should calculate budget', inject([ProductService], (service: ProductService) => {
    let productSpending = [{
      spending: 2
    },
    {
